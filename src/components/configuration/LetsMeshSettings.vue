...
const BROKER_OPTIONS = [
  { value: 0, label: 'Europe only (EU v1)' },
  { value: 1, label: 'US West only (US v1)' },
    ...
    <div class="glass-card rounded-lg border border-stroke-subtle dark:border-stroke/10 p-6">
      <div class="flex items-start justify-between mb-6">
        <div>
          ...
          <p class="text-sm text-content-secondary dark:text-content-muted">
            Configure LetsMesh MQTT observer settings
          </p>
        </div>
        <button
          v-if="!isEditing"
      ...
      <div v-if="!isEditing" class="space-y-4">
        ...
        <div>
          ...
          >
          <div
            v-if="!letsmeshConfig.additional_brokers?.length"
          ...
          <div v-else class="mt-2 space-y-1.5">
            <div
              v-for="b in letsmeshConfig.additional_brokers"
              :key="b.host"
        ...
        <div>
          <span
            class="text-xs font-medium text-content-secondary dark:text-content-muted uppercase tracking-wide"
            >Blocked Packet Types</span
          >
          <div class="mt-2 flex flex-wrap gap-1.5">
            <span
              v-if="!letsmeshConfig.disallowed_packet_types?.length"
              class="text-sm text-content-muted dark:text-content-muted/60 italic"
              >All types allowed</span
            >
            <span
              v-for="t in letsmeshConfig.disallowed_packet_types"
              :key="t"
      ...
      <div v-else class="space-y-5">
        ...
        >
          <div>
            <label class="text-sm font-medium text-content-primary dark:text-content-primary"
              >Enable LetsMesh Observer</label
            >
            <p class="text-xs text-content-secondary dark:text-content-muted mt-0.5">
              Publish mesh packets to the LetsMesh MQTT network
            </p>
          </div>
          <button
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            >
            <select
              v-model.number="brokerIndexInput"
        ...
        >
          ...
          A service restart is required for LetsMesh changes to take effect.
        </div>
